...
public class BankAccount
{
    ...
    public bool Withdraw(decimal amount, string inputPin)
    {
        if (IsLocked)
        {
            Console.WriteLine("Withdrawal failed: Account is locked.");
            return false;
        }
        ...
        {
            _failedAttempts++;
            Console.WriteLine($"Withdrawal failed: Incorrect PIN. Attempt {_failedAttempts} of 3.");
    ...
    public decimal GetBalance(string inputPin)
    {
        if (inputPin != _pin)
        {
            Console.WriteLine("GetBalance failed: Incorrect PIN.");
            return -1m;
        }
    ...
    public bool ChangePin(string currentPin, string newPin)
    {
        if (IsLocked)
        {
            Console.WriteLine("Change PIN failed: Account is locked.");
            return false;
        }

        if (currentPin != _pin)
        {
            Console.WriteLine("Change PIN failed: Incorrect current PIN.");
            return false;
        }
